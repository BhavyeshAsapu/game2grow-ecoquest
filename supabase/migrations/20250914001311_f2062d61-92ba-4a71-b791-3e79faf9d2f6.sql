...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_rewards_updated_at'
  ) THEN
...
-- Purchases policies
CREATE POLICY IF NOT EXISTS "Users can view their own reward purchases"
...
SELECT 'Eco Workbook', 'Printable activities on recycling and energy saving.', 100, 'https://images.unsplash.com/photo-1516979187457-637abb4f9353'
WHERE NOT EXISTS (SELECT 1 FROM public.rewards);

INSERT INTO public.rewards (title, description, cost, image_url)
SELECT 'Science E-Book', 'Intro to environmental science for teens.', 250, 'https://images.unsplash.com/photo-1526304640581-d334cdbbf45e'
WHERE (SELECT COUNT(*) FROM public.rewards) < 2;

INSERT INTO public.rewards (title, description, cost, image_url)
SELECT 'Plantable Seed Paper', 'Seed paper for a small home planting project.', 400, 'https://images.unsplash.com/photo-1524593137-9f3fd3fbffc8'
